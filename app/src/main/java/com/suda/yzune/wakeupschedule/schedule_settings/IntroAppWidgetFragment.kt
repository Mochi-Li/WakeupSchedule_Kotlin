...
class IntroAppWidgetFragment : BaseListFragment() {
    ...
    private fun onItemsCreated(items: MutableList<BaseSettingItem>) {
        ...
        items.add(VerticalItem(R.string.setting_resize_widget, "如果想调整小部件整体的高度，要在桌面长按小部件来调整。华为和荣耀手机如果长按后调整不了，是第三方主题导致的，请切换回系统默认主题再调整。"))
        items.add(VerticalItem(R.string.setting_widget_style, "小部件右上角有个「调整」的按钮，点它就可以了。"))
        items.add(VerticalItem(R.string.setting_widget_loading, "可能是被手机清理了后台。请在系统设置中，手动管理本 App 的后台，允许本 App 后台自启和后台运行。另外，小部件右上角有个小箭头，点击两次可以强制刷新，不需要重新放置小部件的。" +
                "\n" +
                "华为/荣耀手机设置方式：打开系统自带的手机管家 -> 应用启动管理 -> 找到 WakeUp课程表 -> 设置为手动管理后台，同时允许后台自启和后台运行。"))
        items.add(VerticalItem(R.string.setting_more_question, "根据反馈不定时更新"))
        items.add(VerticalItem(R.string.setting_blank, "\n\n\n"))
    }

    private fun onVerticalItemClick(item: VerticalItem) {
        when (item.title) {
            R.string.setting_more_question -> Utils.openUrl(requireActivity(), "https://support.qq.com/embed/97617/faqs-more")
        }
    }
}