...
// 重庆医科大学
class SupwisdomCQMUParser(source: String) : Parser(source) {
    ...
    override fun generateCourseList(): List<Course> {
        ...
        for (findText in foundResults) {
            ...
            if (line.contains("new TaskActivity(")) {
                val a = line.split(',')
                val groupName = a[a.size - 3].removeSurrounding("\"")
                courseName = if (groupName.isNotBlank()) {
                    a[a.size - 13].removeSurrounding("\"") + "(${groupName})"
                } else {
                    a[a.size - 13].removeSurrounding("\"")
                }
                room = a[a.size - 11].removeSurrounding("\"")
                val weekStr = a[a.size - 10].removeSurrounding("\"")
