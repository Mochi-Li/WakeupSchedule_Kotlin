...
// 辽宁工程技术大学
class SupwisdomLNTUParser(source: String) : Parser(source) {
    ...
    override fun generateCourseList(): List<Course> {
        ...
        for (findText in foundResults) {
            ...
            if (line.contains("new TaskActivity(")) {
                val a = line.split(',')
                val groupName = a[a.size - 2].removeSurrounding("\"")
                courseName = if (groupName.isNotBlank()) {
                    a[a.size - 10].removeSurrounding("\"").substringBeforeLast('(') + "(组${groupName})"
                } else {
                    a[a.size - 10].removeSurrounding("\"").substringBeforeLast('(')
                }
                room = a[a.size - 8].removeSurrounding("\"")
                val weekStr = a[a.size - 7].removeSurrounding("\"")
