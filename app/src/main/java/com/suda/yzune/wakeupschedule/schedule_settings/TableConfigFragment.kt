...
class TableConfigFragment : BaseListFragment() {
    ...
    private val currentWeekItem by lazy(LazyThreadSafetyMode.NONE) {
        SeekBarItem("当前周", viewModel.getCurrentWeek(), 1, viewModel.tableConfig.maxWeek, "周", "第")
    }
...
//                is VerticalItem -> onVerticalItemClick(item)
    ...
    private fun onItemsCreated(items: MutableList<BaseSettingItem>) {
        items.add(HorizontalItem("课表名称", viewModel.tableConfig.tableName))
        items.add(HorizontalItem("上课时间", "点击此处更改"))
        items.add(HorizontalItem("学期开始日期", viewModel.tableConfig.startDate))
        items.add(currentWeekItem)
        items.add(SeekBarItem("一天课程节数", viewModel.tableConfig.nodes, 1, 30, "节"))
        items.add(SeekBarItem("学期周数", viewModel.tableConfig.maxWeek, 1, 60, "周"))
        items.add(SwitchItem("周日为每周第一天", viewModel.tableConfig.sundayFirst))
        items.add(HorizontalItem("管理已添加课程", ""))
...
//        items.add(SwitchItem("显示周日", viewModel.tableConfig.showSun))
    ...
    private fun onSwitchItemCheckChange(item: SwitchItem, isChecked: Boolean, position: Int) {
        when (item.title) {
            "周日为每周第一天" -> viewModel.tableConfig.sundayFirst = isChecked
        }
    ...
    private fun onSeekBarItemClick(item: SeekBarItem, position: Int) {
        Utils.showSeekBarItemDialog(requireContext(), item) { dialog, valueInt ->
            when (item.title) {
                "一天课程节数" -> viewModel.tableConfig.nodes = valueInt
                "学期周数" -> {
